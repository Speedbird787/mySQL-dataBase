...
-- task 1 - total num of employees  - query answer= 18
 SELECT COUNT(*) FROM employee_detail; 

-- task 2 - highest earning employee with jobtitle - query answer = marketing lead, 75,000
SELECT employee_detail.employee_id, name, job_title, salary
...
-- task 3 - count devs in each wage bracket - query answer =  35,000 - 40,000 = 4 40,000-48,000 = 4
SELECT name, job_title, salary
FROM current_job_detail JOIN employee_detail
ON employee_detail.employee_id = current_job_detail.employee_id
WHERE job_title LIKE "Developer" ORDER BY salary DESC;

-- task 4 - list non-devs and their salarys - query answer = 
SELECT name, job_title, salary
FROM current_job_detail JOIN employee_detail
ON employee_detail.employee_id = current_job_detail.employee_id
WHERE job_title NOT LIKE "Developer" ORDER BY salary DESC;

-- stretch task - multiple tables to movies with join queries 
